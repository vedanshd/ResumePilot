...
export function CoverLetterViewer({ 
  ...
  const improveMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest("POST", `/api/covers/${id}/improve`);
    },
    onSuccess: (response) => {
  ...
  const generateClientSidePDF = () => {
    try {
          ...
          item.text.forEach((textPart: any) => {
            currentText += textPart.text || textPart;
          });
          const lines = doc.splitTextToSize(currentText, 170);
          doc.text(lines, 20, yPosition);
          yPosition += lines.length * 5 + (item.margin?.[3] || 0);
        }
      ...
      doc.save(response.filename || `cover-letter-${company || 'document'}.pdf`);
      
      toast({
        title: "Exported to PDF",
        description: "Cover letter has been exported as a PDF.",
      });
    },
    onError: () => {
      ...
      >
        ...
        <div className="mt-4 flex-1 flex flex-col min-h-0">
          ...
          <div 
            ...
            style={{ color: colors.mutedForeground }}
          >
            {displayContent.split(/\s+/).filter(word => word.length > 0).length} words
          </div>
        </div>
      </DialogContent>
    </Dialog>
